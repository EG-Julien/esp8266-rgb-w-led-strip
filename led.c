...
void led_init() {
    ...
    gpio_enable(led_white_gpio, GPIO_OUTPUT);
    led_write(led_white_on);
}
...
void led_white_identify(homekit_value_t _value) {
    printf("LED identify\n");
    xTaskCreate(led_identify_task, "LED identify", 128, NULL, 2, NULL);
}

homekit_value_t led_white_on_get() {
    return HOMEKIT_BOOL(led_white_on);
}

void led_white_on_set(homekit_value_t value) {
    if (value.format != homekit_format_bool) {
        printf("Invalid value format %d\n", value.format);
        return;
    }

    led_white_on = value.bool_value;
    led_write(led_white_on);
}
...
homekit_accessory_t *accessories[] = {
    ...
    HOMEKIT_ACCESSORY(.id=2, .category=homekit_accessory_category_lightbulb, .services=(homekit_service_t*[]){
        HOMEKIT_SERVICE(ACCESSORY_INFORMATION, .characteristics=(homekit_characteristic_t*[]){
            HOMEKIT_CHARACTERISTIC(NAME, "White LED Strip"),
            HOMEKIT_CHARACTERISTIC(MANUFACTURER, "Kariboo"),
            HOMEKIT_CHARACTERISTIC(SERIAL_NUMBER, "037A2BABF19D"),
            HOMEKIT_CHARACTERISTIC(MODEL, "Kariboo LED Strip"),
            HOMEKIT_CHARACTERISTIC(FIRMWARE_REVISION, "0.1"),
            HOMEKIT_CHARACTERISTIC(IDENTIFY, led_white_identify),
            NULL
        }),
        HOMEKIT_SERVICE(LIGHTBULB, .primary=true, .characteristics=(homekit_characteristic_t*[]){
            HOMEKIT_CHARACTERISTIC(NAME, "White LED Strip"),
            HOMEKIT_CHARACTERISTIC(
                ON, false,
                .getter=led_white_on_get,
                .setter=led_white_on_set
            ),
            NULL
        }),
        NULL
